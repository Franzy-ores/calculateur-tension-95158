  ...
  return (
    <div className="flex items-center justify-between px-4 py-2 border-b border-primary/20">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-base font-bold text-primary">Calcul Chute de Tension BT</h1>
        </div>
        {editTarget === 'simulation' && (
          <Badge variant="default" className="animate-pulse bg-accent text-accent-foreground text-xs px-2 py-0.5">
            🔬 Mode Simulation
          </Badge>
        )}
      </div>
      ...
      {currentProject && (
        <div className="flex items-center gap-2">
          <Badge 
            variant="outline" 
            className={`${currentProject.voltageSystem === 'TÉTRAPHASÉ_400V' ? 'border-primary bg-primary/10 text-primary' : 'border-secondary bg-secondary/10 text-secondary'} text-xs px-2 py-0.5`}
          >
          ...
          {hasSimulationEquipment && (
            <Badge 
              variant={isSimulationActive ? "default" : "outline"} 
              className={`text-xs px-2 py-0.5 ${isSimulationActive ? 'bg-success text-success-foreground' : 'border-muted-foreground/30 text-muted-foreground'}`}
            >
